...
The `Schema` and `Reference` classes are Pydantic models that represent JSON Schema objects in Python. They are used as the input to the [converter](https://danipulok.github.io/pydantic-jsonschema/0.0.13/converters/index.md).
...
`Schema` fields map directly to JSON Schema keywords. Other or custom keywords are preserved via `extra="allow"`. The converter consumes every declared keyword (`format` is annotation-only unless a matching entry is passed in [`formats`](https://danipulok.github.io/pydantic-jsonschema/0.0.13/formats/index.md), per the spec). Unknown or custom keywords round-trip through parsing and serialization without affecting the generated model.
...
`if` / `else` are Python reserved words, so the fields use a trailing underscore with an alias. `populate_by_name=True` keeps both forms loadable (`Schema(if_=...)` and `{"if": ...}`).
...
- [Converters](https://danipulok.github.io/pydantic-jsonschema/0.0.13/converters/index.md) - Convert schemas to Pydantic models
- [Formats](https://danipulok.github.io/pydantic-jsonschema/0.0.13/formats/index.md) - Add validation for `format` values
- [API Reference](https://danipulok.github.io/pydantic-jsonschema/0.0.13/api/schema/index.md) - Full API docs for schema types
